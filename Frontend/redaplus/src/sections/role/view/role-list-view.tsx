...
import { IRole, PermissionSlug, IRoleTableFilters } from 'src/types/role';
...
const STATUS_OPTIONS = [{ value: '', label: 'Todos', dataCy: 'All' }, ...CATEGORY_STATUS_OPTIONS];
...
export default function RoleListView() {
  ...
  const [data, setData] = useState<IPaginated<IRole>>(defaultPaginated);
  const [loading, setLoading] = useState<boolean>(false);

  const { filters, handleFilterChange, applyFilters, resetFilters } = useFilter<IRoleTableFilters>({
  ...
  const handleActiveRows = async () => {
    try {
      ...
      const promises = selectedRows.map((row) =>
        axiosInstance.put(`${endpoints.role}/${row.id}`, {
          active: true,
        })
      );
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        ...
        await axiosInstance.delete(`${endpoints.role}/${id}`);
        enqueueSnackbar(successDeleteText('Função', false));

        setData((old) => ({ ...old, results: deleteRow }));

        table.onUpdatePageDeleteRow(data?.results?.length || 0);
      } catch (error) {
        enqueueSnackbar(failDeleteText('Função'), { variant: 'error' });
      }
    },
  ...
  return (
    <>
      <Container maxWidth={settings.themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Lista de funções', href: paths.dashboard.role.root },
          ]}
          action={
            can(PermissionSlug['add-roles']) && (
              <Button
                component={RouterLink}
                href={paths.dashboard.role.new}
                variant="contained"
                startIcon={<Iconify icon="mingcute:add-line" />}
                data-cy="list-new-role"
              >
                Nova Função
              </Button>
            )
      ...
      <ConfirmDialog
        ...
        title="Ativar funções selecionadas"
        content={
          <>
      ...
      <ConfirmDialog
        ...
        title="Inativar funções selecionadas"
        content={
          <>
