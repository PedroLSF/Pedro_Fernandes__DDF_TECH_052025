...
export default function VideoPicker({
  ...
  const handleAddToIntro = (item: ItemProps) => {
    setSelected([item]);
    setAddOrRemove('introduction');
  };

  const handleAddToEnding = (item: ItemProps) => {
    setSelected([item]);
    setAddOrRemove('ending');
  };
  ...
  return (
    <Card {...other}>
      <Box display="flex" flexDirection="row" justifyContent="space-between" alignItems="center">
        ...
        {currentTab === ChannelEditTabs.Group &&
          dataFiltered.some(
            (item) =>
              !item.videoChannels.some((channel) => channel.channel_id === currentChannel?.id)
          ) && (
            <IconButton
              color="primary"
          ...
          {isLoading ? (
            ...
            dataFiltered.map((news) => (
              <NewsItem
                ...
                onAddToIntro={handleAddToIntro}
                onAddToEnding={handleAddToEnding}
                onRemoveFromPicked={handleRemoveFromPicked}
                currentTab={currentTab}
                isPicked={
                  currentPlaylist
                    ? news?.videoPlaylist?.some(
                        (channel) => channel.playlist_id === currentPlaylist?.id
                      )
                    : news?.videoChannels?.some(
                        (channel) => channel.channel_id === currentChannel?.id
                      )
                }
...
type NewsItemProps = {
  ...
  onAddToIntro: (item: ItemProps) => void;
  onAddToEnding: (item: ItemProps) => void;
...
function NewsItem({
  ...
  const handleAddToIntro = () => {
    onAddToIntro(news);
  };

  const handleAddToEnding = () => {
    onAddToEnding(news);
  };
    ...
    >
      ...
      >
        ...
        {currentTab !== ChannelEditTabs.Group ? (
          ...
          >
            {currentTab === 'intro' && (
              <IconButton color="primary" onClick={handleAddToIntro}>
                <Iconify icon="mingcute:add-line" data-cy={`add-${title}-video-picker`} />
              </IconButton>
            )}
            {currentTab === 'ending' && (
              <IconButton color="primary" onClick={handleAddToEnding}>
                <Iconify icon="mingcute:add-line" data-cy={`add-${title}-video-picker`} />
              </IconButton>
            )}
          </Box>
        ...
        {!isPicked && currentTab === ChannelEditTabs.Group && (
          <Box
            sx={{
              pr: 3,
            }}
