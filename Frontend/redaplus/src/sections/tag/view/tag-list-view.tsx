...
import { ITagItem, ITagTableFilters, TAG_STATUS_OPTIONS } from 'src/types/tag';
...
const STATUS_OPTIONS = [{ value: '', label: 'Todos' }, ...TAG_STATUS_OPTIONS];
...
const defaultFilters: ITagTableFilters = {
  name: '',
  color: '',
  active: '',
  end_date: null,
...
export default function TagListView() {
  ...
  const [data, setData] = useState<IPaginated<ITagItem>>(defaultPaginated);
  const [loading, setLoading] = useState<boolean>(false);

  const { filters, handleFilterChange, applyFilters, resetFilters } = useFilter<ITagTableFilters>({
  ...
  const handleActiveRows = async () => {
    try {
      ...
      const promises = selectedRows.map((row) =>
        axiosInstance.put(`${endpoints.tag}/${row.id}`, {
          active: true,
        })
      );
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        ...
        await axiosInstance.delete(`${endpoints.tag}/${id}`);
        enqueueSnackbar(successDeleteText('Tag', false));

        setData((old) => ({ ...old, results: deleteRow }));

        table.onUpdatePageDeleteRow(data?.results?.length || 0);
      } catch (error) {
        enqueueSnackbar(failDeleteText('tag'), { variant: 'error' });
      }
    },
  ...
  return (
    <>
      <Container maxWidth={settings.themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Lista de Tags"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'Lista de Tags', href: paths.dashboard.tag.root },
          ]}
          action={
            can(PermissionSlug['add-tag']) && (
              <Button
                component={RouterLink}
                href={paths.dashboard.tag.new}
                variant="contained"
                startIcon={<Iconify icon="mingcute:add-line" />}
                data-cy="list-new-tag"
              >
                Nova Tag
              </Button>
            )
      ...
      <ConfirmDialog
        ...
        title="Ativar tags selecionadas"
        content={
          <>
      ...
      <ConfirmDialog
        ...
        title="Inativar tags selecionadas"
        content={
          <>
